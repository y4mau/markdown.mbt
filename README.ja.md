...
<p align="center">
  ...
  <br />
  <em>プレイグラウンド — Markdown のリアルタイム編集とプレビュー</em>
</p>
...
- **フローティングドキュメントスイッチャー** — 最近開いたドキュメント一覧のサイドバー、FLIP アニメーション付き並び替え、ファイルピッカーアップロード、キーボードナビゲーション（`Ctrl+Shift+P`）
- **スクロール位置の保持** — エディタ・プレビュー双方のスクロール位置がリロードやドキュメント切り替え後も復元されます
- **相対パス解決** — 相対パスのリンクや画像（例: `./docs/arch.md`、`./img/screenshot.png`）がプレビューで正しく解決されます
...
- **テーブル → チャート可視化** — 数値だけで構成された Markdown テーブルを Chart.js のインタラクティブなチャートとして描画（棒・線・円グラフの切り替え可）
- **ネストした Markdown の描画** — `markdown` / `md` 言語指定のフェンスドコードブロックを完全に描画された Markdown として表示（ドキュメント用サンプルに便利）
- **GitHub 風 diff ハイライト** — `diff` 言語指定のコードブロックで追加/削除行の背景色と、選択不可の `+` / `-` ガター付きで表示
...
- **mdpreview のタブ再利用** — 連続して `mdpreview` を実行したときに新しいタブを開かず、既存タブを再利用
...
- [MoonBit](https://www.moonbitlang.com/download/) — インストール後にターミナルを再起動
